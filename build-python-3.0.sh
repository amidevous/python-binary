...
TARGET=/usr/local/python3.0
TARGET_ARCHIVE_DIR=/app/build
TARGET_ARCHIVE_PATH=${TARGET_ARCHIVE_DIR}/python3.0-x86-64.tar.gz
...
cd /tmp
wget --no-check-certificate https://www.python.org/ftp/python/3.0.1/Python-3.0.1.tgz
tar -xzf Python-*.tgz
cd Python-*
...
rm -rf ${TARGET}/lib/python3.0/test
